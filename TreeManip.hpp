...
namespace strom {
    ...
    class TreeManip {
        ...
        public:
            ...
            void storeSplit(std::set<Split> & splitset);
    ...
    inline void TreeManip::extractNodeNumberFromName(Node * nd, std::set<unsigned> & used) {
        ...
        if (success) {
            ...
            if (insert_result.second) {
                ...
                throw XStrom(boost::str(boost::format("leaf number %d used more than one") % x));
            }
        }
        else
    ...
    inline void TreeManip::renumberInternals() {
        ...
        _tree->_nintervals = curr_internal - _tree->_nleaves;
    ...
    inline void TreeManip::storeSplit(std::set<Split> & splitset) {
